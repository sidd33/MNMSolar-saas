...
import { Shield, FastForward, ArrowRight, ListTodo, Clock, Eye, DownloadCloud, Settings, Split, CheckCircle2, UploadCloud, AlertCircle } from "lucide-react";
...
import { useEngineeringNexus } from "../dashboard/EngineeringNexusProvider";
...
export function EngineeringHandoffCard({ project, dept, initialFiles }: EngineeringHandoffCardProps) {
  const { refresh, updateLocalProject } = useEngineeringNexus();
  ...
  const checklistComplete = isSiteSurveyStage
    ? !!surveyReportFile
    : (surveyVerified && sldUploaded && layoutUploaded && structuralVerified && bomReady);
  ...
  const handleFileUpload = async (tag: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await uploadFiles(project.id, [taggedFile], "TECHNICAL" as any, null, (savedFiles) => {
    ...
    <Card className="overflow-hidden border border-slate-100 shadow-xl bg-white [contain:paint] will-change-transform group rounded-[2.5rem] transition-all hover:shadow-2xl hover:border-[#1C3384]/20">
      <div className="grid grid-cols-1 lg:grid-cols-3">
            ...
            {isSiteSurveyStage ? (
              ...
              <>
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                   ...
                   <div className={cn("flex items-center gap-2 font-black text-sm", checklistComplete ? "text-emerald-500" : "text-yellow-500")}>
                     <span className="tabular-nums">{completedCount}/5</span> Verified
                   </div>
                </div>
        ...
        <div className="bg-[#1C3384] p-10 text-white relative flex flex-col justify-center border-l-4" style={{ borderColor: checklistComplete ? "#10B981" : "transparent" }}>
          ...
          <div className="mb-8 p-5 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-md relative z-10">
